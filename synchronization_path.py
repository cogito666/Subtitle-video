...
def dpcore(M, pen):
    ...
    for c in range(1, np.shape(M)[1]):
        for r in range(1, np.shape(M)[0]):
            best_preceding_costs = [D[r - 1, c - 1], pen + D[r, c - 1],
                                    pen + D[r - 1, c]]
...
def smoothing_curve(target_path, ref_path):
    """
    path smoother

    :param target_path: target video path point
    :param ref_path: reference video path point
    :return: smoothed_target_path, smoothed_ref_path
    """
    smoothed_target_path = savgol_filter(target_path, window_length=11, polyorder=3)
    smoothed_ref_path = savgol_filter(ref_path, window_length=11, polyorder=3)

    smoothed_target_path = np.round(smoothed_target_path, decimals=0)
    smoothed_ref_path = np.round(smoothed_ref_path, decimals=0)
